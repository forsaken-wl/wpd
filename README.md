...
All settings live in `${XDG_CONFIG_HOME:-$HOME/.config}/wpd/switcher.conf`. Daemon keys are `transition`, `duration_ms`, `fps`, and `matugen_command`. The seven transition names are `fade`, `wipe`, `slide`, `grow`, `center`, `corner`, and `infection`.
