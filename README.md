...
All settings live in `${XDG_CONFIG_HOME:-$HOME/.config}/wpd/switcher.conf`. Daemon keys are `transition`, `duration_ms`, `fps`, and `matugen_command`. Use `wpd transition`, `wpd transition list`, or `wpd transition MODE`; changes are persisted and sent to the running daemon immediately. Available modes are `fade`, `wipe`, `slide`, `grow`, `center`, `corner`, `infection`, `radial`, `diamond`, `blinds`, `checker`, `wave`, `curtain`, `clock`, `dissolve`, and `sweep`.
